...
The benchmark requires two small libraries for command line and config parsing. The download and build script for these libraries is placed in `src/ASYNC/thirdparty` directory. It must download, build and place the resulting files in a right place. The benchmark build code will link these libraries statically into the resulting benchmark binary.
